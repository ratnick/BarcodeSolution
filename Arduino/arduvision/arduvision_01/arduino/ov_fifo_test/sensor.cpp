...
#include <Arduino.h>
#include <Wire.h>
#include "LogLib.h"    // you must also include Time and TimeLib
#include <TimeLib.h>  
#include <Time.h>
...
uint8_t sensor_readReg(uint8_t regID) {
   uint8_t regDat;
   LogLinef(2, __FUNCTION__, "A");
   Wire.beginTransmission(OV772x_RD_ADDR >> 1);
   LogLinef(2, __FUNCTION__, "B");
   Wire.write(regID & 0x00FF);
   LogLinef(2, __FUNCTION__, "C");
   Wire.endTransmission();
   LogLinef(2, __FUNCTION__, "D");
